...
class Engine:
    ...
    def __call__(self, input_file_path, output_file_path):
        ...
        print('获得分离出的原音频文件：', original_audio_file)
        ...
        print('获得分离出的原音频文件,拆分为人声和噪音')

        speakers, lang = self.transcribe_audio_extended(audio_res['voice_file'])
        print('whisper进行语音识别，得到说话人以及语种信息:', "说话人：", speakers, "语言：", lang)
        # ---------------------------------------------------------------------------------------------------
        ...
        print('voice clone done,file_name:', combined_audio)
        ...
        if orig_clip is not None:
            ...
            new_video_clip = self.video_map_audio_speed(speech_audio, orig_clip)
        ...
        if platform.system() == 'Windows':
            subprocess.call("copy {} {}/voice_cloned.wav".format(combined_audio, Path(output_file_path).parent), shell=True)
        else:
            subprocess.call("cp {} {}/voice_cloned.wav".format(combined_audio, Path(output_file_path).parent),
                            shell=True)
        ...
        if platform.system() == 'Windows':
            command = 'pip install librosa==0.9.2 &&  cd ./video-retalking  && python inference.py \
    ...
    # 该方法调整视频速度以匹配音频时长
    def video_map_audio_speed(self, audio, video):
        audio_duration = audio.duration_seconds
        video_duration = video.duration
        ratio = audio_duration / video_duration
        speed = video_duration / audio_duration
        print("音频duration:", audio_duration)
        print("视频duration:", video_duration)
        print("audio_duration / video_duration=ratio:{}".format(ratio))
        new_video = video.fx(vfx.speedx, speed)
        # result_a.write_videofile("output_video_a.mp4", codec="libx264", audio=False)
        return new_video
